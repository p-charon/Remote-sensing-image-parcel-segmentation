...
class Predictor:
    ...
    def output_result(self, imgs_data, infer_out, ims ,  use_pr=False):
        for idx in range(len(imgs_data)):
            ...
            #cv2.imshow("after" , result_png)
            #cv2.moveWindow("trans1:" + ims,1000,100) 
            #cv2.moveWindow("trans2:" + result_png,1000,100) 
            #cv2.waitKey(0)
            #cv2.destroyAllWindows()
            result_png = cv2.medianBlur(result_png,7)
    ...
    def predict(self, images):
        ...
        for i in range(0, len(images), batch_size):
            ...
            img_datas = self.image_reader.process(images[i:i + real_batch_size],
                                                  use_pr)
            ims = self.image_reader.process_reader(images[i:i + real_batch_size],
                                                  use_pr)
            ...
            self.output_result(img_datas, output_data , ims, use_pr)
            post_end = time.time()
